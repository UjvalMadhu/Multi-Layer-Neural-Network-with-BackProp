...
/*
void backProp_H(double ** Y, double** Y_O, double** Y_1, double** WO, double** W1_updt, int S, int UL_m2, int UL_m1, int UL) {
	...
	for (int sam = 0; sam < S; sam++) {
		for (int i = 0; i < UL_m2; i++) {
			for (int j = 0; j < UL_m1; j++) {
				double sum_Cost = 0.0;
				for (int u = 0; u < UL; i++) {
					sum_Cost += (Y[sam][u] / Y_O[sam][u]) * (Y_O[sam][u] * (1 - Y_O[sam][u])) * WO[j + 1][u];
				}
				W1_updt[i][j] += sum_Cost * (Y_1[sam][j] * (1 - Y_1[sam][j])*
			}
		}
	}
